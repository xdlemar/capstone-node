...
describe("E2E: Procurement + PLT (shipment tied to PO) via Gateway", () => {
  ...
  test("staff cannot access PLT projects", async () => {
    const staffAuth = makeAuthz("STAFF");
    await agent
      .post("/api/plt/projects")
      .set(staffAuth)
      .send({ code: `PRJ-STAFF-${Math.floor(Math.random() * 1e9)}`, name: "No Access" })
      .expect(403);
  });
  ...
  test("create PR -> approve -> PO", async () => {
    ...
    const pr = await agent
      ...
      .send({
        prNo,
        notes: "Radiology supplies",
        lines: [{ itemId: ITEM_ID, qty: 100, unit: "box" }],
      })
      .expect(200);
    expect(pr.body.status).toBe("SUBMITTED");

    const approved = await agent
      .post(`/api/procurement/pr/${prNo}/approve`)
      .set(authz)
      .expect(200);
  ...
  test("PLT: create project, then delivery linked to PO", async () => {
    const proj = await agent
      .post("/api/plt/projects")
      .set(authz)
      .send({ code: `PRJ-${Math.floor(Math.random() * 1e9)}`, name: "Radiology Upgrade" })
      .expect(200);
    S.project = proj.body;

    const del = await agent
      .post("/api/plt/deliveries")
      .set(authz)
      .send({
  ...
  test("Procurement: post a Receipt for the PO (after PLT delivered)", async () => {
    ...
    const rc = await agent
      ...
      .set(authz)
      .send({ poNo: S.po.poNo, drNo, invoiceNo: `INV-${drNo}` })
      .expect(201);
